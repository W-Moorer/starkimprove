...
#include <stark>
#include "paths.h"
#include "rb_constraint_test_scenes.h" // Contains a bunch of simple rigid body scenes
#include "paper_experiments/paper_experiments.h"
...
int main(int argc, char** argv)
{
	...
	*/
	...
	symx::compiler_command = "\"C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\VC\\Auxiliary\\Build\\vcvars64.bat\"";

	const std::string experiment = (argc > 1) ? std::string(argv[1]) : "exp1";
	if (experiment == "exp1") {
		exp1_collision_free_and_stiffness();
		return 0;
	}
	if (experiment == "exp1_adaptive") {
		exp1_adaptive_only();
		return 0;
	}
	if (experiment == "exp1_gap_adaptive") {
		exp1_gap_adaptive_only();
		return 0;
	}
	if (experiment == "exp1_mass_adaptive") {
