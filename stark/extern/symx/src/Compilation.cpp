...
void symx::Compilation::compile(const std::vector<Scalar>& expr, std::string name, std::string folder, std::string id, OpType op_type, bool suppress_compiler_output)
{
	...
	std::string shell_command = symx::compiler_command + enable_output;
	shell_command += " && cd " + folder;
	shell_command += " && cl " + name + ".cpp /LD /Ox /arch:AVX2 /bigobj" + enable_output;  // Note: fast-math is not used because it can change the expected results (e.g. sqrt(pow(x, 2)) < 0)
	shell_command += " && del " + name + ".exp";
	shell_command += " && del " + name + ".lib";
	shell_command += " && del " + name + ".obj";
