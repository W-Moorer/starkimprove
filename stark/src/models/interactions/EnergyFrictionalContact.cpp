...
void stark::EnergyFrictionalContact::_add_rigid_body(const RigidBodyHandler& rb, const Handler& handler, const std::vector<Eigen::Vector3d>& vertices)
{
	...
	// Store body mass for this contact group
	while ((int)this->contact_group_masses.size() <= group) {
		this->contact_group_masses.push_back(1.0);  // default mass for earlier groups
	}
	this->contact_group_masses[group] = rb.get_mass();
...
void EnergyFrictionalContact::_before_energy_evaluation__update_contacts(core::Stark& stark)
{
	...
	this->last_min_contact_distance = std::numeric_limits<double>::infinity();
	this->last_min_contact_dhat = std::numeric_limits<double>::infinity();
	this->last_critical_reduced_mass = 1.0;
	this->last_active_contact_pairs = 0;

	// Auto-initialize kappa from system mass properties (once)
	if (this->global_params.inertia_consistent_kappa && !this->kappa_auto_initialized) {
		this->kappa_auto_initialized = true;
		double m_max = 0.0;
		double dhat_min = std::numeric_limits<double>::infinity();
		for (int i = 0; i < (int)this->contact_group_masses.size(); ++i) {
			m_max = std::max(m_max, this->contact_group_masses[i]);
		}
		for (int i = 0; i < (int)this->contact_thicknesses.size(); ++i) {
			dhat_min = std::min(dhat_min, this->contact_thicknesses[i]);
		}
		if (m_max > 0.0 && std::isfinite(dhat_min) && dhat_min > 0.0) {
			const double g = 9.81;  // gravitational reference acceleration
			const double C = 1.0;   // safety factor
			const double kappa_auto = C * m_max * g / (dhat_min * dhat_min);
			const double kappa_init = std::max(this->global_params.min_contact_stiffness, kappa_auto);
			stark.console.print(fmt::format("Inertia-consistent kappa auto-init: m_max={:.2e}, dhat_min={:.2e}, kappa_0={:.2e}\n",
				m_max, dhat_min, kappa_init), core::ConsoleVerbosity::Frames);
			this->contact_stiffness = kappa_init;
		}
	}
	...
	auto update_contact_diagnostics = [&](double distance, int set_a, int set_b)
		{
			double dhat = this->_get_contact_distance(set_a, set_b);
			if (distance > dhat) {
				return;
			}
			this->last_active_contact_pairs++;
			if (distance < this->last_min_contact_distance) {
				this->last_min_contact_distance = distance;
				this->last_min_contact_dhat = dhat;
				// Compute reduced mass of this critical pair
				double m_a = (set_a < (int)this->contact_group_masses.size()) ? this->contact_group_masses[set_a] : 1.0;
				double m_b = (set_b < (int)this->contact_group_masses.size()) ? this->contact_group_masses[set_b] : 1.0;
				this->last_critical_reduced_mass = (m_a * m_b) / (m_a + m_b);
			}
		};

	for (const auto& pair : proximity.point_triangle.point_point) {
		update_contact_diagnostics(pair.distance, pair.first.set, pair.second.point.set);
	}
	for (const auto& pair : proximity.point_triangle.point_edge) {
		update_contact_diagnostics(pair.distance, pair.first.set, pair.second.edge.set);
	}
	for (const auto& pair : proximity.point_triangle.point_triangle) {
		update_contact_diagnostics(pair.distance, pair.first.set, pair.second.set);
	}
	for (const auto& pair : proximity.edge_edge.point_point) {
		update_contact_diagnostics(pair.distance, pair.first.edge.set, pair.second.edge.set);
	}
	for (const auto& pair : proximity.edge_edge.point_edge) {
		update_contact_diagnostics(pair.distance, pair.first.edge.set, pair.second.set);
	}
...
{
	if (!this->global_params.stiffness_update_enabled) {
		return;
	}
	const double old_stiffness = this->contact_stiffness;

	if (this->global_params.adaptive_stiffness_scheduling
		&& std::isfinite(this->last_min_contact_distance)
		&& std::isfinite(this->last_min_contact_dhat)
		&& this->last_min_contact_dhat > 0.0)
	{
		// Gap-ratio-driven adaptive increase
		constexpr double c_u = 1.0;       // increase sensitivity
		constexpr double eps = 0.01;       // gap ratio floor
		constexpr double alpha_max = 8.0;  // maximum amplification factor
		const double r = std::clamp(this->last_min_contact_distance / this->last_min_contact_dhat, 0.0, 1.0);
		double alpha = std::min(alpha_max, 1.0 + c_u / (r + eps));

		// Mass-ratio correction: heavier critical pairs need more stiffness
		if (this->global_params.inertia_consistent_kappa && this->last_critical_reduced_mass > 0.0) {
			// Compute reference reduced mass as the average body mass
			double m_sum = 0.0;
			int n_groups = (int)this->contact_group_masses.size();
			for (int i = 0; i < n_groups; ++i) m_sum += this->contact_group_masses[i];
			double m_ref = (n_groups > 0) ? (m_sum / n_groups) : 1.0;
			// Reduced mass of a uniform pair: m_ref * m_ref / (2*m_ref) = m_ref/2
			double m_star_ref = m_ref * 0.5;
			if (m_star_ref > 0.0) {
				double beta = std::sqrt(this->last_critical_reduced_mass / m_star_ref);
				alpha = std::min(alpha_max, alpha * std::max(1.0, beta));
			}
		}

		this->contact_stiffness *= alpha;
		stark.console.add_error_msg(fmt::format(
			"Penetration couldn't be avoided. Adaptive stiffness hardened from {:.1e} to {:.1e} (gap ratio r={:.4f}, alpha={:.2f}, m*={:.2e}).",
			old_stiffness, this->contact_stiffness, r, alpha, this->last_critical_reduced_mass));
	}
	else {
		// Simple fallback: fixed doubling
...
{
	stark.logger.append_to_series("contact_active_pairs", this->last_active_contact_pairs);
	if (std::isfinite(this->last_min_contact_distance)) {
		stark.logger.append_to_series("contact_min_distance", this->last_min_contact_distance);
	}
	else {
		stark.logger.append_to_series("contact_min_distance", -1.0);
	}
	stark.logger.append_to_series("contact_stiffness", this->contact_stiffness);
	stark.logger.append_to_series("contact_critical_reduced_mass", this->last_critical_reduced_mass);
	...
	if (this->global_params.adaptive_stiffness_scheduling) {
		// Gap-ratio-driven adaptive decay
		constexpr double c_d = 0.05;      // decay rate coefficient
		constexpr double c_free = 0.10;    // decay rate when no contacts active

		if (this->last_active_contact_pairs > 0
			&& std::isfinite(this->last_min_contact_distance)
			&& std::isfinite(this->last_min_contact_dhat)
			&& this->last_min_contact_dhat > 0.0)
		{
			const double r = std::clamp(this->last_min_contact_distance / this->last_min_contact_dhat, 0.0, 1.0);
			this->contact_stiffness = std::max(this->global_params.min_contact_stiffness,
				this->contact_stiffness * (1.0 - c_d * r));
		}
		else {
			// No active contacts: relax faster
			this->contact_stiffness = std::max(this->global_params.min_contact_stiffness,
				this->contact_stiffness * (1.0 - c_free));
		}
	}
	else {
		// Simple rule: fixed 1% decay
