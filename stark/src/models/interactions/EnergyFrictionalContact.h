...
namespace stark
{
	...
	public:
		...
		struct GlobalParams
		{
			...
			STARK_PARAM_NO_VALIDATION(bool, stiffness_update_enabled, true)
			STARK_PARAM_NO_VALIDATION(bool, adaptive_stiffness_scheduling, false)  // Gap-ratio-driven adaptive κ scheduling
			STARK_PARAM_NO_VALIDATION(bool, inertia_consistent_kappa, false)  // Auto-init κ from system mass properties
		};
		struct Params
		{
	...
	private:
		...
		// Per-contact-group body mass (populated at registration time)
		std::vector<double> contact_group_masses;
		bool kappa_auto_initialized = false;

		// Per-accepted-step contact diagnostics (updated before each energy evaluation)
		double last_min_contact_distance = std::numeric_limits<double>::infinity();
		double last_min_contact_dhat = std::numeric_limits<double>::infinity();  // dhat of the pair achieving d_min
		double last_critical_reduced_mass = 1.0;  // reduced mass of the pair achieving d_min
		int last_active_contact_pairs = 0;
